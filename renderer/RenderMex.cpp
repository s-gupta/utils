...
//   glEnd();
  ...
  plhs[1] = mxCreateNumericMatrix(m_width, m_height, mxUINT32_CLASS, mxREAL);
  unsigned int* resultColor = (unsigned int*) mxGetData(plhs[1]);
 
  unsigned int* resultCur = resultColor;
  unsigned int* resultEnd = resultColor + m_width * m_height;
