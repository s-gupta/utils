function forest = forestTrainFn( treeData, hs, varargin )
...
%
% USAGE
%  forest = forestTrainFn( treeData, hs, [varargin] )
%
% INPUTS
%  treeData     - a structure with functions: look at matrixToTreeData.m
%   .getNumPoints
%   .getFeatureDim
%   .samplePoints
%   .getMatrixValues
...
assert(length(hs)==N); discr=~isempty(discretize);
minChild=max(1,minChild); minCount=max([1 minCount minChild]);
if(isempty(H)), H=max(hs); end; assert(discr || all(hs>0 & hs<=H), 'Assertion: "discr || all(hs>0 & hs<=H)" failed.');
...
for i=1:M
  if(N==N1), treeData1=treeData; hs1=hs; dWts1=dWts; 
  else
    d=wswor(dWts,N1,4); 
    
    % data1=data(d,:); 
    treeData1 = treeData.samplePoints(treeData, d);
    
    hs1=hs(d);
    dWts1=dWts(d); 
    dWts1=dWts1/sum(dWts1);
  end
  tree = treeTrain(treeData1,hs1,dWts1,prmTree);
  if(i==1), forest=tree(ones(M,1)); else forest(i)=tree; end
end

end

function tree = treeTrain( treeData, hs, dWts, prmTree )
...
K=2*N-1; discr=~isempty(discretize);
...
while( k < K )
  ...
  data1 = single(treeData.getMatrixValues(treeData, dids1, fids1));
