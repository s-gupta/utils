function [hs,ps] = forestApplyFn( data, forest, maxDepth, minCount, best )
...
%N=size(data,1);
N = data.getNumPoints(data);
F = data.getFeatureDim(data);
%dataAll = single(data.getMatrixValues(data, 1:N, 1:F));
...
for i=1:M, 
  ...
  %ids = forestInds(dataAll, tree.thrs, tree.fids, tree.child, 4);
  
  fids = tree.fids+1;  %% fids is 0 based indexing, as we just found out!
  fidsUniq = unique(fids(fids > 0));
  [~, locFidsUnique] = ismember(fids, fidsUniq);
  dataAll = single(data.getMatrixValues(data, 1:N, double(fidsUniq)));
  locFidsUnique = uint32(locFidsUnique);
  % keyboard;
  % ind = fids > 0; tt = [fidsUniq(locFidsUnique(ind)), fids(ind)];
  % [isequal(tt(:,1), tt(:,2)), isequal(locFidsUnique > 0, fids > 0)]
  % [max(locFidsUnique) size(dataAll, 2)]
  % sum(dataAll(:))
  assert(max(locFidsUnique) <= size(dataAll, 2), 'Something wrong with taking unique features... \n');
  ids = forestInds(dataAll, tree.thrs, locFidsUnique-1, tree.child, 1);
  leaf_ids(:,i) = ids;
